...
if __name__ == '__main__':
    ...
    while 1:
        ...
        n = 0
        err = 0
        rand = randint(0,len(X_test),1)##SGD
        for i in range(len(rand)):
            a = nn.forward_propagation(X_test[rand[i]])
            b = nn.backward_propagation(t_test[rand[i]], a)
            grad_i2h += nn.calc_gradient(b[0], X_test[rand[i]])
